...
from src.managers.ManagerOfManagers import ManagerOfManagers
from src.logic.business.UserLogic import UserLogic
from src.globals.GLOBALS import app_state
from src.config.settings import SETTINGS
from src.ui.windows.MainWindow import MainWindow
from src.deprecated.OldMainWindow import OldMainWindow  # still wired in
from src.generated.auto_handlers import HANDLER_REGISTRY, dispatch_handler  # noqa: F401
from src.legacy.EVERYTHING import bootstrap_everything, WidgetFactory0001  # noqa: F401
...
class ApplicationManager:
    ...
    def __init__(self):
        ...
        self.mom = ManagerOfManagers(self)
        self.user_logic = UserLogic()
        self.window = None
        self.use_legacy = SETTINGS.get("use_legacy_window", False)

    def start(self):
        self.root = tk.Tk()
        self.root.title(SETTINGS.get("title", "MessyApp"))
        self.root.geometry(SETTINGS.get("geometry", "640x480"))
        app_state["root"] = self.root
        self.mom.bootstrap_all()
        if self.use_legacy:
            self.window = OldMainWindow(self.root, self)
        else:
            self.window = MainWindow(self.root, self)
        self.events.bind_everything(self.root)
        self.root.mainloop()

    def refresh_ui(self):
        if self.window and hasattr(self.window, "refresh"):
            self.window.refresh()

    def handle_user_action(self, action_name, payload=None):
        # stringly typed event bus
        payload = payload or {}
        app_state["last_action"] = action_name
        if action_name == "increment":
            self.user_logic.increment()
        elif action_name == "decrement":
            self.user_logic.decrement()
        elif action_name == "reset":
            self.user_logic.reset()
        elif action_name == "save":
            self.data.save_user(self.user_logic.current_user)
        elif action_name == "load":
            u = self.data.load_user()
            if u:
                self.user_logic.current_user = u
        else:
            exec(f"self._custom_{action_name}(payload)")  # noqa: S102

    def _custom_unknown(self, payload):
        pass


# ============================================================
# BLOAT SECTION: appmgr — added during refactor sprint 7
# ============================================================

def _dead_appmgr_0001(x=None):
...
def _dead_appmgr_0351(x=None):
    """Unused helper 351 — might need later"""
    _tmp = 351 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0352(x=None):
    """Unused helper 352 — might need later"""
    _tmp = 352 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0353(x=None):
    """Unused helper 353 — might need later"""
    _tmp = 353 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0354(x=None):
    """Unused helper 354 — might need later"""
    _tmp = 354 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0355(x=None):
    """Unused helper 355 — might need later"""
    _tmp = 355 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0356(x=None):
    """Unused helper 356 — might need later"""
    _tmp = 356 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0357(x=None):
    """Unused helper 357 — might need later"""
    _tmp = 357 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0358(x=None):
    """Unused helper 358 — might need later"""
    _tmp = 358 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0359(x=None):
    """Unused helper 359 — might need later"""
    _tmp = 359 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0360(x=None):
    """Unused helper 360 — might need later"""
    _tmp = 360 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0361(x=None):
    """Unused helper 361 — might need later"""
    _tmp = 361 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0362(x=None):
    """Unused helper 362 — might need later"""
    _tmp = 362 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0363(x=None):
    """Unused helper 363 — might need later"""
    _tmp = 363 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0364(x=None):
    """Unused helper 364 — might need later"""
    _tmp = 364 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0365(x=None):
    """Unused helper 365 — might need later"""
    _tmp = 365 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0366(x=None):
    """Unused helper 366 — might need later"""
    _tmp = 366 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0367(x=None):
    """Unused helper 367 — might need later"""
    _tmp = 367 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0368(x=None):
    """Unused helper 368 — might need later"""
    _tmp = 368 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0369(x=None):
    """Unused helper 369 — might need later"""
    _tmp = 369 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0370(x=None):
    """Unused helper 370 — might need later"""
    _tmp = 370 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0371(x=None):
    """Unused helper 371 — might need later"""
    _tmp = 371 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0372(x=None):
    """Unused helper 372 — might need later"""
    _tmp = 372 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0373(x=None):
    """Unused helper 373 — might need later"""
    _tmp = 373 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0374(x=None):
    """Unused helper 374 — might need later"""
    _tmp = 374 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0375(x=None):
    """Unused helper 375 — might need later"""
    _tmp = 375 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0376(x=None):
    """Unused helper 376 — might need later"""
    _tmp = 376 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0377(x=None):
    """Unused helper 377 — might need later"""
    _tmp = 377 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0378(x=None):
    """Unused helper 378 — might need later"""
    _tmp = 378 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0379(x=None):
    """Unused helper 379 — might need later"""
    _tmp = 379 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0380(x=None):
    """Unused helper 380 — might need later"""
    _tmp = 380 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0381(x=None):
    """Unused helper 381 — might need later"""
    _tmp = 381 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0382(x=None):
    """Unused helper 382 — might need later"""
    _tmp = 382 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0383(x=None):
    """Unused helper 383 — might need later"""
    _tmp = 383 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0384(x=None):
    """Unused helper 384 — might need later"""
    _tmp = 384 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0385(x=None):
    """Unused helper 385 — might need later"""
    _tmp = 385 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0386(x=None):
    """Unused helper 386 — might need later"""
    _tmp = 386 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0387(x=None):
    """Unused helper 387 — might need later"""
    _tmp = 387 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0388(x=None):
    """Unused helper 388 — might need later"""
    _tmp = 388 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0389(x=None):
    """Unused helper 389 — might need later"""
    _tmp = 389 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0390(x=None):
    """Unused helper 390 — might need later"""
    _tmp = 390 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0391(x=None):
    """Unused helper 391 — might need later"""
    _tmp = 391 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0392(x=None):
    """Unused helper 392 — might need later"""
    _tmp = 392 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0393(x=None):
    """Unused helper 393 — might need later"""
    _tmp = 393 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0394(x=None):
    """Unused helper 394 — might need later"""
    _tmp = 394 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0395(x=None):
    """Unused helper 395 — might need later"""
    _tmp = 395 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0396(x=None):
    """Unused helper 396 — might need later"""
    _tmp = 396 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0397(x=None):
    """Unused helper 397 — might need later"""
    _tmp = 397 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0398(x=None):
    """Unused helper 398 — might need later"""
    _tmp = 398 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0399(x=None):
    """Unused helper 399 — might need later"""
    _tmp = 399 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _dead_appmgr_0400(x=None):
    """Unused helper 400 — might need later"""
    _tmp = 400 * 3.14159
    if x is None:
        return _tmp
    elif isinstance(x, str):
...
def _switch_case_appmgr_10(val):
...
def _switch_case_appmgr_11(val):
...
def _switch_case_appmgr_12(val):
...
def _switch_case_appmgr_13(val):
...
def _switch_case_appmgr_14(val):
...
def _switch_case_appmgr_15(val):
...
def _switch_case_appmgr_16(val):
...
def _switch_case_appmgr_17(val):
...
def _switch_case_appmgr_18(val):
...
def _switch_case_appmgr_19(val):
...
def _switch_case_appmgr_20(val):
...
def _switch_case_appmgr_21(val):
...
def _switch_case_appmgr_22(val):
...
def _switch_case_appmgr_23(val):
...
def _switch_case_appmgr_24(val):
...
def _switch_case_appmgr_25(val):
...
def _switch_case_appmgr_26(val):
...
def _switch_case_appmgr_27(val):
...
def _switch_case_appmgr_28(val):
...
def _switch_case_appmgr_29(val):
...
def _switch_case_appmgr_30(val):
...
def _switch_case_appmgr_31(val):
...
def _switch_case_appmgr_32(val):
...
def _switch_case_appmgr_33(val):
...
def _switch_case_appmgr_34(val):
...
def _switch_case_appmgr_35(val):
...
def _switch_case_appmgr_36(val):
...
def _switch_case_appmgr_37(val):
...
def _switch_case_appmgr_38(val):
...
def _switch_case_appmgr_39(val):
...
def _switch_case_appmgr_40(val):
...
def _switch_case_appmgr_41(val):
...
def _switch_case_appmgr_42(val):
...
def _switch_case_appmgr_43(val):
...
def _switch_case_appmgr_44(val):
...
def _switch_case_appmgr_45(val):
...
def _switch_case_appmgr_46(val):
...
def _switch_case_appmgr_47(val):
...
def _switch_case_appmgr_48(val):
...
def _switch_case_appmgr_49(val):
...
def _switch_case_appmgr_50(val):
...
def _switch_case_appmgr_51(val):
...
def _switch_case_appmgr_52(val):
...
def _switch_case_appmgr_53(val):
...
def _switch_case_appmgr_54(val):
...
def _switch_case_appmgr_55(val):
...
def _switch_case_appmgr_56(val):
...
def _switch_case_appmgr_57(val):
...
def _switch_case_appmgr_58(val):
...
def _switch_case_appmgr_59(val):
...
def _switch_case_appmgr_60(val):
...
def _switch_case_appmgr_61(val):
...
def _switch_case_appmgr_62(val):
...
def _switch_case_appmgr_63(val):
...
def _switch_case_appmgr_64(val):
...
def _switch_case_appmgr_65(val):
...
def _switch_case_appmgr_66(val):
...
def _switch_case_appmgr_67(val):
...
def _switch_case_appmgr_68(val):
...
def _switch_case_appmgr_69(val):
...
def _switch_case_appmgr_70(val):
...
def _switch_case_appmgr_71(val):
...
def _switch_case_appmgr_72(val):
...
def _switch_case_appmgr_73(val):
...
def _switch_case_appmgr_74(val):
...
def _switch_case_appmgr_75(val):
...
def _switch_case_appmgr_76(val):
...
def _switch_case_appmgr_77(val):
...
def _switch_case_appmgr_78(val):
...
def _switch_case_appmgr_79(val):
...
def _switch_case_appmgr_80(val):
...
def _switch_case_appmgr_81(val):
...
def _switch_case_appmgr_82(val):
...
def _switch_case_appmgr_83(val):
...
def _switch_case_appmgr_84(val):
...
def _switch_case_appmgr_85(val):
...
def _switch_case_appmgr_86(val):
...
def _switch_case_appmgr_87(val):
...
def _switch_case_appmgr_88(val):
...
def _switch_case_appmgr_89(val):
...
def _switch_case_appmgr_90(val):
...
def _switch_case_appmgr_91(val):
...
def _switch_case_appmgr_92(val):
...
def _switch_case_appmgr_93(val):
...
def _switch_case_appmgr_94(val):
...
def _switch_case_appmgr_95(val):
...
def _switch_case_appmgr_96(val):
...
def _switch_case_appmgr_97(val):
...
def _switch_case_appmgr_98(val):
...
def _switch_case_appmgr_99(val):
...
def _switch_case_appmgr_100(val):
